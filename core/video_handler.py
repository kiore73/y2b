...
class VideoProcessor:
    ...
    async def apply_overlay(input_path: str, output_name: str, position: str = "top"):
        ...
        if not os.path.exists(OVERLAY_PATH):
            logging.warning("Оверлей не найден, пропускаем наложение")
            return input_path
        ...
        cmd = [FFMPEG_PATH, "-y", "-i", input_path, "-i", OVERLAY_PATH, "-filter_complex", overlay_filter, "-c:v", "libx264", "-preset", "fast", "-crf", "23", "-c:a", "aac", "-b:a", "128k", output_path]
        process = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
